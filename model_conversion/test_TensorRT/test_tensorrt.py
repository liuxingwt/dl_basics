...
def load_normalized_test_case(test_images, pagelocked_buffer, preprocess_func):
    ...
    print("data before into buffer:",data[:10])
...
def infer(engine_path, preprocess_func, batch_size=1, input_images=[]):
    with open(engine_path, 'rb') as f, trt.Runtime(trt.Logger(trt.Logger.WARNING)) as runtime:
        ...
        with engine.create_execution_context() as context:
            ...
            print("image array before into pfld.engine:", test_images.shape)
            ...
            batch_outs = np.reshape(batch_outs, (-1,2))
            np.savetxt("result.txt", batch_outs)
            print("result:", batch_outs)
...
def get_inputs(filename=None, directory=None, allowed_extensions=(".bmp", ".jpeg", ".jpg", ".png")):
    filenames = []
    if filename:
...
if __name__ == '__main__':
    ...
    if args.preprocess_func is not None:
        ...
        preprocess_func = processing.preprocess_pfld
